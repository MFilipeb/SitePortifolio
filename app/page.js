    ...
    <div className="theme-wrapper" style={customStyles} data-wallpaper={profile.wallpaperStyle || 'image'}>
      ...
      <main className="linktree-container">
        ...
        <section className={`profile-section layout-${profile.profileLayout || 'classic'}`}>
          {profile.titleStyle === 'logo' && profile.avatarUrl ? (
            <img src={profile.avatarUrl} alt="Logo" className="profile-logo" />
          ) : (
        ...
        <section className="links-grid">
          {links.map((link) => (
            <a key={link.id} href={link.title.includes('Meus Projetos') ? '/projetos' : link.url} target={link.title.includes('Meus Projetos') ? '_self' : '_blank'} rel="noopener noreferrer" className="link-card" data-style={profile.btnStyle || 'outline'}>

              {/* Optional Left Icon based on Data */}
              {(link.iconType && link.iconType !== 'link') && (
                <div className="link-icon">
                  <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(5px)', borderRadius: '50%', padding: '6px', display: 'flex', border: '1px solid rgba(255,255,255,0.2)' }}>
